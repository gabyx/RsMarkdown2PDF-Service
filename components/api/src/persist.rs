use common::{job::JobBundle, log::info, storage::BlobStorage};
...
pub async fn create_job_bundle(
    ...
    info!(log, "Persist upload to temporary file.");
    ...
    let (_, digest) = storage.store_blob(&tmp_file, &content_type).await?;
